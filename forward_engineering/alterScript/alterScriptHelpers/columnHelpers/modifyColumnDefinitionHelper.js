...
const shouldModifyColumn = _ => (columnName, columnJsonSchema, collection) => {
	...
	return [wasCommentModified, wasNotNullModified].some(wasModified => wasModified === true);
};
...
const removeKeysFromHydratedColumn = _ => hydratedColumn => {
	return _.omit(hydratedColumn, ['primaryKey', 'primaryKeyOptions', 'unique', 'uniqueKeyOptions']);
};
...
const getModifyColumnDefinitionScriptDtos = (_, ddlProvider) => collection => {
	const { getTableName, wrapInTics, getCollectionSchema, getCollectionName, getDatabaseName } =
