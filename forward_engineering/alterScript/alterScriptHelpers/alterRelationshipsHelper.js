...
const getAddSingleForeignKeyStatementDto = (ddlProvider, _) => relationship => {
	...
	const relationshipName = compMod.code?.new || compMod.name?.new || extractNameFromRelationship(relationship) || '';
...
const canRelationshipBeAdded = relationship => {
	...
	return [
		compMod.code?.new || compMod.name?.new || extractNameFromRelationship(relationship),
...
const getDeleteSingleForeignKeyStatementDto = (ddlProvider, _) => relationship => {
	...
	const relationshipName = compMod.code?.old || compMod.name?.old || extractNameFromRelationship(relationship) || '';
...
const canRelationshipBeDeleted = relationship => {
	...
	return [
		compMod.code?.old || compMod.name?.old || extractNameFromRelationship(relationship),
		compMod.child?.bucket,
		compMod.child?.collection,
	].every(property => Boolean(property));
};
